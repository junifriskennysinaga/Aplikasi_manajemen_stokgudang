@extends('layouts.app')
@section('title', 'Barang Keluar - GudangKu')
...
<div class="space-y-6">
    ...
    <div>
        <h1 class="text-2xl lg:text-3xl font-extrabold text-slate-900 tracking-tight">Barang Keluar</h1>
        <p class="text-slate-400 text-sm mt-1 font-medium">Kelola stok barang yang keluar dari gudang</p>
    </div>
    ...
    <div class="grid gap-3">
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 p-5 hover:shadow-md transition-shadow duration-200">
            ...
            <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-5">
                ...
                <form action="{{ route('barang-keluar.store') }}" method="POST" class="flex flex-wrap gap-3 items-end">

                    @csrf
                    <input type="hidden" name="barang_id" value="{{ $b->id }}">

                    <div>
                        <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wide mb-1">Jumlah Keluar</label>
                        <input type="number" name="jumlah" min="1" max="{{ $b->stok }}" required
                    ...
                    <div>
                        <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wide mb-1">Tanggal Keluar</label>
                    ...
                    <button type="submit"
                        class="bg-rose-600 hover:bg-rose-700 text-white px-5 py-2 rounded-xl font-semibold text-sm transition shadow-lg shadow-rose-600/10">
                        Kurangi
                    </button>
